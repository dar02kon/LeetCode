...
[原题链接](https://leetcode.cn/problems/bu-ke-pai-zhong-de-shun-zi-lcof/description/?favorite=xb9nqhhg)

[测试代码](https://github.com/dar02kon/LeetCode/blob/master/src/com/dar/leetcode/the_sword_refers_to_offer/ACardInAPlayingCard.java)
...
先将数组从小到大进行排序，遍历过程中需要统计0的个数以及缺少数字的个数（如4，6中间缺一个5），每缺一个数字就消耗一个0，如果最终0的个数不为负数，则这5个数字可以组成一个顺子
...
[原题链接](https://leetcode.cn/problems/ba-zi-fu-chuan-zhuan-huan-cheng-zheng-shu-lcof/description/?favorite=xb9nqhhg)

[测试代码](https://github.com/dar02kon/LeetCode/blob/master/src/com/dar/leetcode/the_sword_refers_to_offer/StringConversionInteger.java)
...
1. 去掉前置空格
2. 判断符号
3. 接收字符，拼接字符转换成数字，遇见其它字符就直接退出循环
4. 根据符号返回相应结果

对于第三步，接收字符在拼接前需要进行越界判断，如当前数字已经大于`Integer.MAX_VALUE/10`，拼接后必然会越界；当前数字等于`Integer.MAX_VALUE/10`，接收字符如果大于7就可以做越界处理（虽然负数末尾可以为8，但最终返回的都是`Integer.MIN_VALUE`，不影响结果）
